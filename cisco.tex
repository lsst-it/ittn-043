...
\begin{itemize}
   \item We already have the Cisco hardware required for network-redesign (Cisco Nexus C9504 and C9508), We only need to adquiere the licensing for NXOS software.
   \item NXOS is a well-known operating software with a good amount of resources available regarding documentation, bugs, forums, security hazzards and more
   \item Rubin's IT network team already have experience with Cisco NXOS environments.
   \item No need for re-route cables at both sites.
\end{itemize}

\subsubsection{Cons}

\begin{itemize}
   \item NXOS licenses for summit and base are pretty expensive, and change the hardware for brand new models with NXOS from fabric is even way more expensive than we thought it will be. 
   \item Cisco NXOS have several bugs and CVEs informed up to date. Cisco is the most attacked networking platform.
\end{itemize}
